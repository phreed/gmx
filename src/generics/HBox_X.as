package generics 
{
	 ...
	 */
	public class HBox_X extends HBox implements ISelfBuilding
	{		
		...
		public function build(xml:XML):void {
			...
			if (xml.@verticalAlign != undefined) { this.setStyle("verticalAlign", xml.@verticalAlign.toString()); }
			...
			if (xml.@type != undefined) {
				switch(xml.@type.toString()) {
					case "buttonGroup":
						this.setStyle("horizontalGap", Size.MM);
						break;
				}
			}
			if (xml.@horizontalGap != undefined) {
				this.setStyle("horizontalGap", GMXComponentBuilder.parseMM(xml.@horizontalGap.toString()));
			}
		...
		public var widthDirty:Boolean = false;
		...
		public function resizeFlexibles():void {
			...
			var i:int;
			var inflexibleComponentTotalWidth:Number = 0;
			...
			var totalPaddingLeftRight:Number = this.getStyle("paddingLeft") + this.getStyle("paddingRight");
			for (i = 0; i < numChildren; i++) {
				inflexibleComponentTotalWidth += horizontalGap;
				if (this.getChildAt(i) is ISelfBuilding) {
					selfBuilding = this.getChildAt(i) as ISelfBuilding;
					if (!selfBuilding.flexible) {
						inflexibleComponentTotalWidth += selfBuilding.width;
					} else {
						flexibleSelfBuildingComponents.push(selfBuilding);
					}
				}
			}
			inflexibleComponentTotalWidth += totalPaddingLeftRight;
			var widthLeft:Number = this.height - inflexibleComponentTotalWidth;
			if ( widthLeft < 0) { 
				//Alert.show("WARNING: ResizingBoxContainer does not have enough room by " + heightLeft + " pixels!");
				 widthLeft = 0;
			}
			var widthPerFlexibleComponent:Number =  widthLeft / flexibleSelfBuildingComponents.length;
			
			for (i = 0; i < flexibleSelfBuildingComponents.length; i++) {
				flexibleSelfBuildingComponents[i].width = widthPerFlexibleComponent;
			}
		}
